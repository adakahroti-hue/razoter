...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const { providerId, providerName, model, monthlyLimit, resetDay, apiKeyName } = body;

    if (!providerId || !providerName) {
      return withCors(NextResponse.json({ error: 'Provider ID and name required' }, { status: 400 }));
    }

    const quota = await addQuota(providerId, providerName, model || '', monthlyLimit || 0, resetDay || 1, apiKeyName || '');
